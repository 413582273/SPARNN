...
https://www.cnblogs.com/wenzb/p/10899313.html
"""
import torch as t
...
file = '../../../../../data/SST/sst.day.mean.2019.nc'
ncdata =nc.Dataset(file)
lat=np.array(ncdata.variables['lat'])
lon=np.array(ncdata.variables['lon'])
sst=np.array(ncdata.variables['sst'])
time=np.array(ncdata.variables['time'])
import sample as sp

lon_range=[122.375,132.125]
lat_range=[21.125,30.875]
# lon_range=[122.375,124.9]
# lat_range=[21.125,24.8]
time_range=[1,360]
time_interval=1 #采样的时间间隔
test_interval=10 #每次测试的时间间隔
drange_lon=4
drange_lat=4
...
# datafile='kssolution256x0-200t05.mat'
...
for q in range(360):
    print(q)
    #计算每次预测的初始时间
    timenow_julian=q*test_interval;
    #计算每次预测的结束时间
    timeend_julian=q*test_interval+\
        (trainstep+predstep)*time_interval-1;
    timerange=[timenow_julian,timeend_julian]
            
    #根据每个局部的计算块，提取数据
    # for p in range(math.floor((lon_range[1]-lon_range[0])/0.25)):
    #     for q in range(math.floor((lat_range[1]-lat_range[0])/0.25)):
    #         latrange=[0,0]
    #         lonrange=[0,0]
    #         #根据计算块提取数据，以参考点为中心，步长为0.25度
    #         #计算中心点
    #         centerlat=lat_range[0]+(q+1)*0.25;
    #         centerlon=lon_range[0]+(p+1)*0.25;
    #         #根据中心点计算范围
    #         latrange[0]=centerlat-drange_lat/2;
    #         latrange[1]=centerlat+drange_lat/2;
    #         lonrange[0]=centerlon-drange_lon/2;
    #         lonrange[1]=centerlon+drange_lon/2;
        

    #首先提取目标变量的测量值
    latdata,londata,sstdata,timedata=sp.sample_ncdata(lon_range,lat_range,\
     timerange,time_interval,\
         lat,lon,sst,time)
    
        
    shape=np.shape(sstdata)
    shape1=shape[0]
    shape2=shape[1]
    uu_noise=sstdata
    ...
    #得到训练数据集
    dataX, dataY = creat_dataset(uu_noise,0,trainstep,predstep)  #第一维是x 第二维是t
    x_train = dataX #训练数据
    ...
    class RNN(nn.Module):
        def __init__(self):
            ...
            self.lstm = nn.LSTM(trainstep,trainstep,1) #输入数据特征维度，隐藏层维度，LSTM串联个数，第二个LSTM接收第一个的计算结果
            self.out = nn.Linear(trainstep,predstep) #线性拟合，接收数据的维度为6，输出数据的维度为1
        def forward(self,x):
            x1,_ = self.lstm(x)
            a,b,c = x1.shape
    ...
    optimizer = t.optim.Adam(rnn.parameters(),lr = 0.01)
    ...
    print(np.size(result_pred))
    result_real.append(dataY)
    print(np.size(result_real))
