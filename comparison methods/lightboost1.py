...
datafile='kssolution256x0-200t01noise005.mat'
...
while timestart<shape2:
    ...
    model = lgbm.LGBMRegressor(max_depth=5, learning_rate=0.1, n_estimators=100) 
    ...
    for j in range(shape1):
        ...
        for q in range(predstep):
            #输出对比的数据
            yref.append(uu_noise[j,timestart+trainstep+1+q:timestart+trainstep+1+1+q])
