...
name1='.\data\kssolution128x0-200t01';
%load Y;
name=[name1,'.mat'];
load(name,'uu');
Y=uu';
...
block=32;   %每个分块的节点数量
predict_len=20;     % L
INPUT_trainlength=41;         %  length of training data (observed data), m > 2L
...
%         selected_variables_idx=[1:block];              % selected the most correlated variables, [1:90] can be changed by personalized methods
    %     xx=X(3000+ii:size(X,1),selected_variables_idx)';       % after transient dynamics
        ...
        %收集预测数据到数组中
        y_predicition=[y_predicition;union_predict_y_ARNN];
...
%     pause(1);
    ...
    result_y=[result_y,refx];
    result_prey=[result_prey,y_predicition];
    result_error=[result_error,result];
%     result_rmse=[result_rmse,rmse.rmse];
%     result_t=[result_t,t(ii+INPUT_trainlength:ii+predict_len+INPUT_trainlength-2)'];
